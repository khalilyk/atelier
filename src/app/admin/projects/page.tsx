...
import BlockList, { RowsEditor } from "../components/BlockList";
import GenericBlock from "@/app/components/GenericBlock";
import ProjectView from "@/app/projects/ProjectView";
import PreviewFrame from "../components/PreviewFrame";
import { projectSlug, parseTags, projectMeta, galleryRows, sortProjects, type Project } from "@/lib/projects";
...
/** Plain text from the project's blocks, for the automatic listing. */
...
function autoSeo(form: Project) {
  const source = [form.summary, projectMeta(form), bodyText(form.body)].filter(Boolean).join(" ");
...
function SeoSection({ form, set }: { form: Project; set: <K extends keyof Project>(k: K, v: Project[K]) => void }) {
  ...
  return (
    <section className="mt-8 pt-6 border-t border-stone-100">
      ...
      <p className="text-xs text-stone-400 mb-5">
        Written for you from the project. {custom ? "You have edited it — Reset goes back to automatic." : "Left as is, it updates itself as you fill the page in."} This is what Google, ChatGPT and the other answer engines read.
      </p>

      {/* Google-style preview */}
      <div className="rounded-xl border border-stone-100 bg-stone-50/70 p-4 mb-5">
        <p className="text-[11px] text-stone-500">ateliersupplygroup.com.au › projects › {form.slug}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="space-y-4">
          ...
          <div>
            <label className={LABEL}>Page length</label>
            <Gauge n={words} limit={SEO_LIMITS.body} unit="words" hint="a fuller case study ranks better" />
          </div>
        </div>
...
const PREVIEW_KEY = "atelier.projects.preview";
...
export default function ProjectsAdmin() {
  ...
  async function create() {
    setError("");
    const res = await fetch("/api/admin/projects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: newTitle }) });
    const d = await res.json();
    if (!res.ok) { setError(d.error || "Could not create the project"); return; }
    setList((l) => sortProjects([...l, d]));
  ...
  async function save() {
    ...
    setList((l) => sortProjects(l.filter((p) => p.slug !== originalSlug).concat(d)));
    setForm(d); setOriginalSlug(d.slug);
  ...
  async function remove() {
    ...
    await fetch("/api/admin/projects", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug: originalSlug }) });
  ...
  async function duplicate() {
    ...
    const d = await res.json();
    if (!res.ok) { setError(d.error || "Could not copy the project"); return; }
    setList((l) => sortProjects([...l, d]));
    setForm(d); setOriginalSlug(d.slug);
  }
  ...
  if (loading) return <div className="p-8 text-stone-400 text-sm">Loading the projects…</div>;

  const live = list.filter((p) => p.published).length;

  return (
    <div className="p-6 lg:p-8">
      {/* Top bar: project picker, name, actions — everything you need without scrolling. */}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            <button onClick={() => setListOpen((v) => !v)} className="flex items-center gap-2 border border-stone-200 bg-white rounded-xl px-4 py-2.5 text-sm text-stone-800 hover:border-[#b8934a]">
              <span className="truncate max-w-[220px]">{form ? form.title : "All projects"}</span>
            ...
            {listOpen && (
              <div className="absolute left-0 mt-2 w-[340px] max-h-[60vh] overflow-auto bg-white rounded-2xl border border-stone-200 shadow-xl z-40">
                {list.length === 0 ? (
                  <p className="px-5 py-6 text-sm text-stone-400">No projects yet. Use New project to add the first one.</p>
                ) : list.map((p) => (
                  <button key={p.slug} onClick={() => pick(p)}
                    ...
                    <p className="text-[11px] text-stone-400 mt-0.5 truncate">
                      {projectMeta(p) || "No location yet"}{p.tags.length ? ` · ${p.tags.join(", ")}` : ""}
                    </p>
                  </button>
                ))}
              </div>
          ...
          <button onClick={() => { setCreating(true); setError(""); }} className="text-xs uppercase tracking-widest px-4 py-2.5 rounded-xl border border-stone-200 bg-white text-stone-600 hover:border-[#b8934a] hover:text-[#b8934a]">New project</button>

          {form && (
            <p className="text-xs text-stone-400 hidden xl:block">/projects/{form.slug}</p>
          )}
          ...
          <div className="ml-auto flex items-center gap-3">
            {form && (
              <>
                ...
                <a href={`/projects/${form.slug}`} target="_blank" className="text-xs border border-stone-200 bg-white rounded-lg px-3 py-2 text-stone-600 hover:border-[#b8934a] hover:text-[#b8934a]">{form.published ? "View" : "Preview"}</a>
      ...
      {creating && (
        <div className="mt-6 bg-white rounded-2xl border border-[#b8934a]/40 p-6">
          ...
          <input autoFocus value={newTitle} placeholder="e.g. Vaucluse Residence" onChange={(e) => setNewTitle(e.target.value)} className={FIELD} />
          <p className="text-[11px] text-stone-400 mt-2">Web address: /projects/{projectSlug(newTitle) || "…"}</p>
      ...
      {!form ? (
        <div className="mt-6 bg-white rounded-2xl border border-stone-100 p-10 text-center text-stone-400 text-sm">Choose a project from the menu above, or add a new one.</div>
      ) : (
        <div className={`mt-6 grid grid-cols-1 gap-6 items-start ${showPreview ? "lg:grid-cols-2" : "xl:grid-cols-[1fr_340px]"}`}>
          ...
          <div className={`bg-white rounded-2xl border border-stone-100 p-6 min-w-0 ${showPreview ? "lg:col-start-1 lg:row-start-1" : ""}`}>
            ...
            <input
              ...
              placeholder="One line about the project (optional)"
          ...
          <aside className={`bg-white rounded-2xl border border-stone-100 overflow-hidden min-w-0 ${showPreview ? "lg:col-start-1 lg:row-start-2" : "xl:sticky xl:top-24"}`}>
            <Panel title="Publishing" open={open === "publishing"} onToggle={() => toggle("publishing")}
              summary={`${form.published ? "Live" : "Draft"}${form.comingSoon ? " · Coming soon" : ""}${form.order ? ` · position ${form.order}` : ""}`}>
              <Toggle label="Published" checked={form.published} onChange={(v) => set("published", v)} hint="Drafts are hidden from the Projects page and from Google." />
              ...
              <div>
                ...
                <p className="text-[11px] text-stone-400 mt-1">1 shows first. 0 sorts by year instead.</p>
              </div>
            </Panel>

            <Panel title="Project details" open={open === "details"} onToggle={() => toggle("details")}
              summary={projectMeta(form) || (form.tags.length ? form.tags.join(", ") : "Nothing filled in yet")}>
              <div className="grid grid-cols-2 gap-3">
                <Text label="Location" value={form.location} onChange={(v) => set("location", v)} placeholder="Vaucluse, Sydney" />
                <Text label="Year" value={form.year} onChange={(v) => set("year", v)} placeholder="2025" />
              </div>
              <Text label="Client" value={form.client} onChange={(v) => set("client", v)} hint="The architect, builder or developer. Leave blank to keep it off the page." />
              <Text label="Scope" rows={2} value={form.scope} onChange={(v) => set("scope", v)} hint="What we supplied, e.g. Windows & doors, custom joinery." />
              <Text label="Web address" value={form.slug} onChange={(v) => set("slug", projectSlug(v))} hint={`/projects/${form.slug}`} />
            ...
            <Panel title="Cover photo" open={open === "cover"} onToggle={() => toggle("cover")}
              summary={`${form.coverImage ? "Cover set" : "No cover yet"}${galleryRows(form.gallery).length ? ` · ${galleryRows(form.gallery).length} more` : ""}`}>
              ...
              <div>
                <label className={LABEL}>More photos</label>
                <p className="text-[11px] text-stone-400 mb-2">Shown as thumbnails under the cover. Any of them opens full size.</p>
                <RowsEditor
                  spec={{ key: "gallery", label: "More photos", kind: "rows", columns: [{ label: "Photo", image: true }, { label: "Caption (optional)" }] }}
                  value={form.gallery}
          ...
          {showPreview && (
            <div className="min-w-0 lg:col-start-2 lg:row-start-1 lg:row-span-2 lg:sticky lg:top-24">
              <div className="flex items-center justify-between gap-3 mb-2">
                ...
                <p className="text-[11px] text-stone-400">/projects/{form.slug}</p>
              </div>
              {/* The real project component, fed the unsaved form. */}
              <div className="rounded-2xl border border-stone-200 bg-[#f5f0e8] overflow-hidden">
                ...
                <PreviewFrame maxHeight="calc(100vh - 220px)">
                  <ProjectView project={form} preview />
                </PreviewFrame>
              </div>
            </div>
          )}
